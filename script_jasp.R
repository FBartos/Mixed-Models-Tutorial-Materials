...
### F-table --
print_f <- function(f, df1, df2){
  sprintf("F(%2$s, %3$.1f) = %1$.2f", f, df1, df2)
}

table_terms  <- c("task", "stimulus", "task:stimulus")
table_models <- list(mm2, mm3, mm4)

F_table <- do.call(cbind, lapply(table_models, function(m){
  col <- NULL
  for(term in table_terms){
    if(term %in% row.names(m$anova_table)){
      col <- c(col, print_f(m$anova_table[term, "F"], m$anova_table[term, "num Df"], m$anova_table[term, "den Df"]))
    }else{
      col <- c(col, "")
    }
  }
  return(data.frame(m = col))
}))
rownames(F_table) <- table_terms
xtable::xtable(F_table)

mm3$anova_table["task", "F"]
